<?php
    ...
    function getUserProfile(PDO $conn, int $userId) : array|false {
        $stmt = $conn->prepare("
            SELECT
                u.username,
                u.email,
    ...
    function updateUserProfile(PDO $conn, int $userId, string $username, string $avatarUrl = null): bool {
        $stmt = $conn->prepare("
            UPDATE users
            set username = ?, avatar_url = ?, updated_at = NOW()
            WHERE id = ?
        ");
        return $stmt->execute([$username, $avatarUrl, $userId]);
    }

    function changeUserPassword(PDO $conn, int $userId, string $currentPassword, string $newPassword) {
    ...
    function changeUserEmail(PDO $conn, int $userId, string $newEmail): bool|string {
